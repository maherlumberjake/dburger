...
export default function BurgerDetails() {
	...
	async function sendComment(id: string) {
		...
		if (!comment) {
			...
			try {
				await axioslocal.patch(`menu/${id}`, { comment });
				setCommentMessage("sent successfully");
				setTimeout(() => {
					setCommentMessage(null);
				}, 3000);
			} catch (error) {
	...
	async function likeIt(id: string) {
		setLoadingLike(true);
		setLikeMessage(null);
		try {
			await axioslocal.get(`menu/like/${id}`);
			setLikeMessage("added to your like list");
			setTimeout(() => {
				setLikeMessage(null);
			}, 3000);
		} catch (error) {
